...
The `go-zeromq` project eagerly accepts contributions from the community.

## Introduction


The `go-zeromq` project provides libraries, in pure-Go, to interoperate with C/C++ ZeroMQ libraries.
We would like you to join us in improving `go-zeromq`'s quality and scope.
...
Before your first merge, you will need to be added to the [CONTRIBUTORS](https://github.com/go-zeromq/license/blob/master/CONTRIBUTORS) and [AUTHORS](https://github.com/go-zeromq/license/blob/master/AUTHORS) files.
Open a pull request adding yourself to these files.
All `go-zeromq` code follows the BSD license in the [license document](https://github.com/go-zeromq/license/blob/master/LICENSE).
...
```
zmq4: add support for ROUTER sockets

This CL adds support for ROUTER sockets.
It makes sure interoperability with the C++ library is retained by:
- adding a test-only Cgo socket wrapping the C++ library
- adding C++/Go and Go/C++ tests

Fixes go-zeromq/zmq4#42.
```

If the `CL` modifies multiple packages at the same time, include them in the commit message:

```
zmq4: implement ROUTER socket

bla-bla

Fixes go-zeromq/zmq4#40.
```
...
Files in the `go-zeromq` repository don't list author names, both to avoid clutter and to avoid having to keep the lists up to date.
Instead, your name will appear in the change log and in the [CONTRIBUTORS](https://github.com/go-zeromq/license/blob/master/CONTRIBUTORS) and [AUTHORS](https://github.com/go-zeromq/license/blob/master/AUTHORS) files.
...
If you don't know who to ask, and aren't getting enough review after saying PTAL, then PTAL @go-zeromq/developers will get more attention.
Also note that you do not have to be the pull request submitter to request additional review.

### What Can I Do to Help?

If you are looking for some way to help the `go-zeromq` project, there are good places to start, depending on what you are comfortable with.

- You can [search](https://github.com/issues?utf8=%E2%9C%93&q=is%3Aopen+is%3Aissue+user%3Ago-zeromq) for open issues in need of resolution.
...
- You can suggest and implement new features that you think belong in `go-zeromq`.
